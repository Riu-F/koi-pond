...
function createSparkles(b: Bounds, count: number = NUM_SPARKLES): WaterSparkle[] {
...
function createScene(b: Bounds, opts: SceneConfig) {
  const lilyPads:    LilyPad[]    = [];
  const lilyFlowers: LilyFlower[] = [];
  const reeds:       Reed[]       = [];
  const fish:        Fish[]       = [];
  const sparkles     = createSparkles(b, opts.sparkleCount);
  ...
  /* Density scalers driven by the controls (1 = original authored density). */
  const padCount = (n: number) => Math.max(0, Math.round(n * opts.lilyPadDensity));

  /* Corner clusters — densest */
  addCluster(0.10, 0.12, padCount(12), sc(b, 40), { x: sc(b, 4), y: sc(b, 4) });
  addCluster(0.88, 0.12, padCount(10), sc(b, 35), { x: b.w - sc(b, 4), y: sc(b, 4) });
  addCluster(0.10, 0.85, padCount(10), sc(b, 35), { x: sc(b, 4), y: b.h - sc(b, 4) });
  addCluster(0.88, 0.85, padCount(12), sc(b, 40), { x: b.w - sc(b, 4), y: b.h - sc(b, 4) });

  /* Edge clusters — medium density */
  addCluster(0.40, 0.08, padCount(6), sc(b, 25), { x: b.w * 0.5, y: sc(b, 4) });
  addCluster(0.60, 0.90, padCount(6), sc(b, 25), { x: b.w * 0.5, y: b.h - sc(b, 4) });
  addCluster(0.05, 0.50, padCount(5), sc(b, 20), { x: sc(b, 4), y: b.h * 0.5 });
  addCluster(0.95, 0.50, padCount(5), sc(b, 20), { x: b.w - sc(b, 4), y: b.h * 0.5 });
  ...
  for (let i = 0; i < 8; i++) {
    addCluster(rand(0.05, 0.95), rand(0.05, 0.95), padCount(1), 0);
  }
  ...
  const reedCount = (n: number) => Math.max(0, Math.round(n * opts.reedDensity));
  reedCluster(0.04, 0.06, reedCount(randInt(10, 14)), sc(b, 16));
  reedCluster(0.96, 0.08, reedCount(randInt(9, 13)),  sc(b, 16));
  reedCluster(0.05, 0.94, reedCount(randInt(12, 15)), sc(b, 18));
  reedCluster(0.97, 0.92, reedCount(randInt(11, 14)), sc(b, 18));
  reedCluster(0.22, 0.04, reedCount(randInt(8, 12)),  sc(b, 14));
  reedCluster(0.78, 0.05, reedCount(randInt(8, 12)),  sc(b, 14));

  const fishTiers = buildFishTiers(opts.fishCount);
...
/* Build a fish-tier list of the requested length, preserving the 2:3:2
   hero:medium:small rhythm (count 7 reproduces the original hand-tuned mix). */
function buildFishTiers(count: number): Array<'hero' | 'medium' | 'small'> {
  const tiers: Array<'hero' | 'medium' | 'small'> = [];
  const n = Math.max(0, Math.round(count));
  for (let i = 0; i < n; i++) {
    const r = i % 7;
    tiers.push(r < 2 ? 'hero' : r < 5 ? 'medium' : 'small');
  }
  return tiers;
}
...
export interface KoiPondProps {
  ...
  lilyPadDensity?: number;
  /** Reed density multiplier — 0 clears them, 2 roughly doubles them (default 1). */
  reedDensity?: number;
...
export default function KoiPond({
  ...
  useEffect(() => {
    ...
    const maxRainRipples = Math.max(0, Math.round(MAX_RAIN_RIPPLES * rainIntensity));
